...
module only prepares the directory and serves it:
    ...
    serve(directory, …)   → http://127.0.0.1:<port>/ (opens the browser unless told not to)
...
def serve(directory: Path, port: int = 0, open_browser: bool = True, duration: float | None = None) -> str:
    """Serve `directory` on 127.0.0.1. `port=0` picks a free one. Blocks until
    Ctrl-C, or for `duration` seconds when given (tests). Returns the URL."""
