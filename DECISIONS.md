...
| **Q2** | CLI or web UI first? | Web UI from day one · CLI + JSON, viewer later · CLI only | **CLI + JSON now, static viewer in M6** | `graph.json`/`report.json` are the contract; a viewer bolts on without touching the engine. | `iacsim/viewer/__init__.py:1-13`, `iacsim/reporter/json_.py:1-38` |
...
| **D11** | Bug: a `--profile` set in one run leaked into the next in-process `Config` | Keep the shallow copy · **real deep copy** | **`copy.deepcopy(DEFAULTS)`** | Found by the CLI tests in M6; the first-level dicts were shared with `DEFAULTS`. | `iacsim/core/config.py:116-119` |
...
| **D16** | Cost of a fan-out (Step Functions `Map`, SQS batch) | count × cost · 1 × cost · **waves** | **M2: one copy. M8: `ceil(count / MaxConcurrency)` sequential waves** | 10 items with concurrency 5 is two rounds, not one. **Known cost:** only the top-level Map is read today — fixed in the current pass. | `iacsim/simulator/traversal.py:~135 _walk_steps`, `iacsim/simulator/traversal.py:~165 _map_concurrency` |
...
## 6. Analysis and reports (D38, D44–D47)
...
| **D45** | How does `diff` line things up? | By index · by name · **label + occurrence** | **Nodes by id (`--align-by label` for Terraform-vs-CFN); scenarios by name; hops by label + occurrence; `|Δ| < 0.05 ms` = unchanged** | Works across the two input formats. Cost: a renamed resource shows as removed + added. | `iacsim/differ.py:84`, `iacsim/differ.py:174`, `iacsim/core/models.py:319` |
| **D46** | CI signal for a latency regression | Report only · **non-zero exit** | **`--fail-on-regression 50ms|10%` → exit 2** | That is what "test infrastructure changes" means in a PR check. | `iacsim/cli.py:134`, `iacsim/differ.py:221` |
...
|---|---|---|---|---|---|
| **P1** | A response leg (the reply back to the caller) — what does it cost? | Re-charge everything (today) · re-charge processing only · **charge a named `respond` key, default 0, no cold start** | **`respond` (default 0) + no distance + no cold start** | The forward hop's `warm` is the *whole* invocation as CloudWatch measures it — composing the reply is inside it. Today 42 % of Foosh `poll_status` is this duplicate, and a Lambda that is provably warm was being charged a cold start. | `iacsim/simulator/traversal.py:~236 _charge`, `iacsim/latency/defaults.yaml` |
| **P2** | Step Functions transition cost — once or per state? | Once on entry (today) · **on every hop out of an orchestrator** | **New `transition` cost rule, on by default** | A 12-task workflow was missing ~275 ms. | `iacsim/latency/rules/` (new `transition.py`), `iacsim/core/config.py:39` |
...
| **P5** | Two rules, same pair, different kind (`env_var` READ vs `iam_policy` WRITE) | First wins (today) · **merge** | **`Edge.ops` = union; `kind` by priority `[INVOKE, ROUTE, CONSUME, PUBLISH, READ, WRITE, PEER]` — READ over WRITE** | A request path *reads* by default; `op: write` in a scenario overrides. Order-independent by construction; the hop table says "(also may write)". | `iacsim/core/pipeline.py:53-58`, `iacsim/core/models.py:111` |
| **P6** | Which Map sets the fan-out concurrency? | First top-level Map (today) · **innermost enclosing Map** | **Recurse into Choice/Parallel bodies; use the innermost Map enclosing the target; disagreement → largest + warning; `fanout.concurrency` override in YAML** | On the real CDK template every Map sits inside a Choice, so waves never fired. | `iacsim/simulator/traversal.py:~165 _map_concurrency` |
